...
const BestSelling: React.FC = () => {
  ...
  const handleWishlist = (product: BestSellingProduct) => {
    ...
    if (alreadyAdded) {
      setPopupInfo({ visible: true, type: 'wishlist', productName: product.name });
      return;
    }
  ...
  const handleConfirm = () => {
    ...
    if (popupInfo.type === 'wishlist') navigate('/wishlist');
    ...
    <section className="container mx-auto px-4 py-8 sm:py-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-8 mb-6 sm:mb-8">
        <div>
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-red-500 font-medium text-sm sm:text-base">This Month</h2>
          </div>
          <h3 className="text-xl sm:text-2xl md:text-3xl font-semibold mt-2 sm:mt-4">Best Selling Products</h3>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {bestSellingProduct.map((product) => (
          <div key={product.id} className="group relative bg-white p-3 sm:p-4 rounded-lg"   
>
            ...
            <div className="mt-3 sm:mt-4">
              ...
              <div className="flex items-center gap-2 mt-1.5 sm:mt-2">
                <span className="text-red-500 font-semibold text-sm sm:text-base">${product.currentPrice}</span>
                <span className="text-gray-500 line-through text-xs sm:text-sm">${product.originalPrice}</span>
              </div>
              <div className="flex items-center gap-2 mt-1.5 sm:mt-2">
                <div className="flex">
                  {[...Array(5)].map((_, i) => (
                    <span key={i} className={`text-${i < product.rating ? 'yellow' : 'gray'}-400 text-sm sm:text-base`}>★</span>
                  ))}
                </div>
